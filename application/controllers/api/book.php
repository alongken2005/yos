...
class Book extends MY_Controller {
		...
		if($type) {
			if($type == 'continue' || $type == 'mylist') {
				...
				if($type == 'continue') {
					...
					$result = $this->db->query('SELECT * FROM yos_book WHERE uid='.$this->member['uid'].$where.$pageLimit)->result_array();
	...
	public function getReviews () {
		...
		$reviews = $this->db->query("SELECT username,title,content,score,ctime FROM yos_book_reviews WHERE bid=".$bid.$pageLimit)->result_array();
	...
	public function getMyReview() {
		...
		$review = $this->base->get_data('book_reviews', array('bid'=>$bid, 'uid'=>$this->member['uid']), 'title, score, content')->row_array();
