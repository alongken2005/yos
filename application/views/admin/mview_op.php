...
?>
<h2><?=intval($this->input->get('id')) ? '修改' : '添加'?><div class="operate"><a href="<?=site_url('admin/mview/lists')?>">管理</a></div></h2>
<div class="slider3">
	<form action="<?=site_url('admin/mview/op'.(intval($this->input->get('id')) ? '?id='.intval($this->input->get('id')) : ''))?>" method="POST" enctype="multipart/form-data">
	<table cellspacing="0" cellpadding="0" border="0" class="table1">
		...
		<tr>
			<th><b>*</b> 作者：</th>
			<td>
				<input type="text" name="username" value="<?=set_value('username', isset($row['username']) ? $row['username'] : '')?>" class="input1"/>
...
$(function() {
	KindEditor.ready(function(K) {
		K.create('#intro', {width : '720', height: '500', newlineTag:'br', filterMode : true});
	});
